...
fetch("https://kea2sem-5899.restdb.io/rest/posts/" + articleId, {
  method: "GET",
  headers: {
...
function showComments(comments) {
  ...
  if (comments.length == 0) {
    ...
    copy.querySelector(".text h2").textContent =
      "There are no comments yet. Be the fist!";
    document.querySelector(".comments-sec").appendChild(copy);
  }
}
...
function handleSubmit(e) {
  ...
  fetch(`https://kea2sem-5899.restdb.io/rest/posts/${articleId}/comments`, {
    ...
    .then((res) => res.json())
    .then((data) => {
      const template = document.querySelector(".single-comment-template")
        .content;
      const copy = template.cloneNode(true);
